...
export const getTodayAttendance = async (req, res) => {
  try {
    ...
    let todayWorkingHours = formatWorkingHours(attendance.totalMinutes || 0);
  ...
  try {
    ...
    const [records, totalRecords] = await Promise.all([
      Attendance.find(query)
        .select(
          "dayKey punchInTime punchOutTime totalMinutes attendanceStatus adminApproved remarks createdAt updatedAt"
        )
        .sort({ dayKey: -1 })
        .skip(skip)
        .limit(limit)
    ...
    return res.status(200).json({
      ...
      attendance: records.map((item) => ({
        ...item,
        id: item._id,
        workingHours: formatWorkingHours(item.totalMinutes || 0),
      })),
      summary: {
...
export const getMyLeaveRequests = async (req, res) => {
  try {
    ...
    const [leaveRequests, totalRecords] = await Promise.all([
      LeaveRequest.find({ employee_id })
        .select("leaveType startDate endDate totalDays remarks status adminRemarks createdAt updatedAt")
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .lean(),
      LeaveRequest.countDocuments({ employee_id }),
    ]);
