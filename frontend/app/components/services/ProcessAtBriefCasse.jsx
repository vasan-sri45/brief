...
const ProcessCard = ({ step, index }) => {
  ...
  return (
    <div
      className={`relative w-full md:w-3/4 lg:w-2/4 self-center ${alignClass}`}
    >
      {/* Step Badge */}
      <div
        className="
      ...
      >
        ...
        <ul className="text-[12px] md:text-[13px] text-letter1 leading-relaxed space-y-1.5">
          {step.lines.map((line, i) => (
            <li
              key={i}
              className="text-justify font-lato text-[0.7rem] md:text-[0.8rem] lg:text-[0.9rem] font-semibold"
            >
              {line}
            </li>
          ))}
